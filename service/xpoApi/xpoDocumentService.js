...
const xpoDocumentService = async (dataObject) => {
    try {
      let vApi = await getVendorsApi('www.xpo.com')
      const authToken = await xpoAuth(vApi)
      const url = `https://api.ltl.xpo.com/imaging/1.0/shipments/${dataObject?.trackingNumber}/imaged-docs?imageFormat=PDF&imageType=${dataObject?.docType}`;
      const headers = {
...
module.exports = xpoDocumentService